...
export default function Navigation() {
  ...
  return (
    <>
      {/* Mobile header */}
      <div className="lg:hidden fixed top-0 left-0 right-0 z-50 bg-gradient-to-r from-blue-600 to-indigo-600 shadow-lg safe-area-top">
        <div className="flex items-center justify-between p-4">
          <div className="flex items-center">
            <div className="w-10 h-10 bg-white/20 backdrop-blur-sm rounded-xl flex items-center justify-center text-white font-bold text-lg shadow-md">
              L
            </div>
            <span className="ml-3 text-xl font-bold text-white tracking-tight">Leo&apos;s Coupons</span>
          </div>
          <button
            type="button"
      ...
      {mobileMenuOpen && (
        <div className="lg:hidden fixed inset-0 z-40 bg-black bg-opacity-50 backdrop-blur-sm" onClick={() => setMobileMenuOpen(false)}>
          <div className="fixed inset-y-0 left-0 w-80 bg-white shadow-2xl rounded-r-3xl" onClick={(e) => e.stopPropagation()}>
            <div className="safe-area-top h-full flex flex-col">
              {/* Header */}
              <div className="bg-gradient-to-r from-blue-600 to-indigo-600 p-6 rounded-r-3xl rounded-bl-none">
                <div className="flex items-center">
                  ...
                  <div className="ml-4">
                    <div className="text-lg font-bold text-white">Leo's Coupons</div>
                    <div className="text-blue-100 text-sm">Hi, {profile?.username}! 👋</div>
                  </div>
                </div>
              </div>
                ...
                {allNavigation.map((item) => {
                  ...
                  return (
                    <Link
                      key={item.name}
                      href={item.href}
                      className={`group flex items-center px-4 py-4 text-base font-medium rounded-2xl transition-all duration-200 ${
                        isActive
              ...
              <div className="p-6 border-t border-gray-100 safe-area-bottom">
                <button
                  ...
                  className="w-full flex items-center px-4 py-4 text-base font-medium text-red-600 hover:bg-red-50 rounded-2xl transition-all duration-200"
                >
